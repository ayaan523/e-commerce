...
if (!clientId) {
  console.warn('VITE_GOOGLE_CLIENT_ID is not configured. Google OAuth will not work. See .env.example for setup.');
}
...
function App() {
  ...
  // ==========================================
  // ROUTE 1: THE ATELIER CONTROL PORTAL (ADMIN)
  // ==========================================
  if (isAdminPortal) {
    return (
      <GoogleOAuthProvider clientId={clientId}>
        {!isAdminAuthenticated ? (
          <AdminLogin onLogin={(token) => { localStorage.setItem('aura_admin_token', token); setIsAdminAuthenticated(true); }} />
        ) : (
          <AdminDashboard 
            isOpen={true} 
            adminToken={localStorage.getItem('aura_admin_token')}
            products={products}
  ...
  // ==========================================
  // ROUTE 2: THE AURA PUBLIC STOREFRONT
  // ==========================================
  return (
    <GoogleOAuthProvider clientId={clientId}>
      <div>
        <nav className="navbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 40px' }}>
          <h1 
            ...
            style={{ margin: 0, letterSpacing: '3px', cursor: 'pointer' }}
          >
            AURA
          </h1>
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center', marginLeft: isProfileOpen ? 'auto' : '0' }}>
            {currentUser ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                >
                  {isProfileOpen ? 'Return to Store' : currentUser.email.split('@')[0]}
                </button>
              </div>
            ) : (
